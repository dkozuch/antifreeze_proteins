...
protein="1ucs"
log_file = "log_antifreeze_score_v7.31_ev_" + protein + "_"
ngens=26
nmuts=17

#get fitted scores from log files
scores = []
for gen in range(0,ngens+1):
	if gen == 0:
...
for i in range(1,len(seqs[:,0])):
        for j in range(0,len(seqs[i,:])):
                if seqs[i,j] != seqs[0,j]:
                        mut_list.append([i,j,seqs[0,j],seqs[i,j]])
...
#print seqs		
with open("seq_scores.txt",'w') as w:
	for i in range(0,len(seqs)):
		num_muts = len(mut_list[mut_list[:,0] == str(i)])
		w.write(str(i) + ", " + "".join(seqs[i]) + ", " + str(max_scores[i]) + ", " + str(num_muts) + "\n") 
...
with open(native_fasta,'r') as f:
	for line in f:
		native_seq = "".join(list(line)[:-1])
file = "0/" + log_file + "0.txt"
with open(file,'r') as f:
	for line in f:
		if line.split()[0:6] == ["Number", "of", "residues", "in", "binding", "face:"]:
			nr = float(line.split()[6])
		...
		if line.split()[0:2] == ["Fitted","Score:"]:
			score = float(line.split()[2])
seqs_all = [[0,0,native_seq,nr,hblb,hbln,area,score]]
	...
	for j in range(1,nmuts+1):
		...
		with open(file,'r') as f:
			for line in f:
				...
				if line.split()[0:2] == ["Fitted","Score:"]:
					score = float(line.split()[2])
		seqs_all.append([gen,mutant,seq,nr,hblb,hbln,area,score])
with open("all_sequences.txt",'w') as w:
	for line in seqs_all:
