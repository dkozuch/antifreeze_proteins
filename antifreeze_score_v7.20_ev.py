...
#same as v13 except k_in_plane was increased from 6 to 8
#use projected area instead of sasa area
#use binding atom coordinates, not residue coordinates
#IMPORTANT: pdb file must start with residues numbering at 0, as well as the sasa file
...
def edit_pdb_sasa(sasa_file,surf_atoms,pdb_file):
	'''edit pdb file with occupancy column being '''
...
def edit_pdb_sasa_r(sasa_file,pdb_file):
	'''edit pdb file with occupancy column '''
...
def get_coordinates(pdb_file,surf_atoms,surf_resids):
	'''returns the coordinates of the residues, calculated as the average of all C,N,S,O atoms in the residue'''
...
def get_hbl_times(n_residues):
	...
	for i in range(0,n_residues):
		...
		#popt,pcurv = curve_fit(func,x,y,p0=(0.3,-1,0.3,-0.1,0.3,-0.1))
		#newx = np.array(range(0,1000))
		#newy = func(newx,*popt)

		#if np.min(newy) > hbl_cutoff:
		#	decayTimei = 0
		#	print("WARNING: HBL for residue " + str(i) + " never decayed below cutoff. Setting decay time to 0")
		#else:
		#	f = interpolate.interp1d(newy,newx) #interpolating for x so reverse
		#	decayTimei = f(hbl_cutoff)*10 #sampled every 10 ps
...
def get_res_mutation(max_face_residues,res_coords,decayTimeList):
	...
	for r in mut_res_list:
		...
		mut_hbl_values.append(binding_hbl_value)
	min_hbl_res = mut_res_list[np.argmin(mut_hbl_values)]
	return [mut_res_list,min_hbl_res]
...
print("Residues (index) in binding face: " + str([int(x) for x in max_face_residues]))
...
[mut_res_list,min_hbl_res] = get_res_mutation(max_face_residues,res_coords,decayTimeList)
print("Residues for mutation: " + str([int(x) for x in mut_res_list]))
print("Residue selected for mutation: " + str(int(min_hbl_res)))
